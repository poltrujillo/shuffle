...
class GameModesMultiActivity : AppCompatActivity() {
    val list = mutableListOf<CarouselItem>()
    var token = ""
    private var modeId = 0
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game_modes_multi)
        ...
        carousel.carouselListener = object : CarouselListener {
            override fun onClick(position: Int, carouselItem: CarouselItem) {
                if (carouselItem.caption == "Artist"){
                    modeId = 2
                    val intent = Intent(this@GameModesMultiActivity, ModeArtistMultiActivity::class.java)
                    intent.putExtra("token", token)
                    intent.putExtra("modeId", modeId)
                    startActivity(intent)
                } else if (carouselItem.caption == "Genre") {
                    modeId = 1
                    val intent = Intent(this@GameModesMultiActivity, ModeGenreMultiActivity::class.java)
                    intent.putExtra("token", token)
                    intent.putExtra("modeId", modeId)
                    startActivity(intent)
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    val intent = Intent(this@GameModesMultiActivity, ModePlaylistMultiActivity::class.java)
                    intent.putExtra("token", token)
                    intent.putExtra("modeId", modeId)
